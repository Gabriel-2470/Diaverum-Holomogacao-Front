...
@Component({
  selector: 'app-importacao-paginacao',
  standalone: true,
  imports: [CommonModule, FormsModule, HasRoleDirective],
  providers: [PacienteService, GruposExamesService, UnidadeService],
  templateUrl: './importacao-paginacao.html',
  styleUrl: './importacao-paginacao.scss',
})
export class ImportacaoPaginacao implements OnInit, OnDestroy {
  ...
  totalRegistros = 0; // total retornado pelo servidor (paginação server-side)
  private debounceSearch: any = null;
  resumoServidor = { pendente: 0, transferidos: 0, agendados: 0, erros: 0 };
  ...
  /**
   * Carrega pacientes usando o endpoint paginado /api/agenda-detalhe-paginacao
   * Paginação, filtro de status e busca são todos resolvidos no servidor.
   */
  carregarPaginado(): void {
    this.carregando = true;
    console.log(`🔄 [carregarPaginado] página=${this.paginaAtual} tamanho=${this.itensPorPagina} status=${this.filtroStatus} search=${this.termoBusca}`);

    let params = new HttpParams()
      .set('pageNumber', this.paginaAtual.toString())
      .set('pageSize', this.itensPorPagina.toString());

    if (this.mostrarApenasUnidadeId != null) {
      params = params.set('idUnidade', this.mostrarApenasUnidadeId.toString());
    }

    if (this.termoBusca.trim()) {
      params = params.set('search', this.termoBusca.trim());
    }
    ...
    if (this.filtroStatus !== 'todos' && statusMap[this.filtroStatus] != null) {
      params = params.set('status', statusMap[this.filtroStatus].toString());
    }

    if (this.filtroDataInicio) params = params.set('dataInicio', this.filtroDataInicio);
    if (this.filtroDataFim)    params = params.set('dataFim', this.filtroDataFim);
    if (this.filtroDataInicio || this.filtroDataFim) {
      params = params.set('tipoData', this.filtroTipoData);
    }

    this.http.get<any>(`${environment.apiUrl}/agenda-detalhe-paginacao`, { params }).subscribe({
      next: (response: any) => {
        const paginacao = response?.paginacao;
        const itens: any[] = paginacao?.items || [];
        this.totalRegistros = paginacao?.totalItems ?? itens.length;
        if (response?.resumo) this.resumoServidor = response.resumo;
        console.log(`✅ [carregarPaginado] ${itens.length} registros / total=${this.totalRegistros}`);
        if (itens.length > 0) {
          console.log('📋 [carregarPaginado] Exemplo:', JSON.stringify(itens[0], null, 2));
        }
        this.processarRespostaPaginada(itens);
      ...
      error: (erro: any) => {
        ...
        this.pacientes = [];
        this.todosPacientes = [];
        console.error('❌ [carregarPaginado] Erro:', erro);
        if (erro?.status === 502) {
  ...
  /**
   * Processa a resposta já agrupada do endpoint paginado.
   * Cada item já possui { idAgendamento, nomePaciente, cpf, exames[], ... }.
   */
  private processarRespostaPaginada(itens: any[]): void {
    const pacientesUI: PacienteUI[] = itens.map((item, index) => {
      const todosEnviados = item.indRegEnviado === true || item.statusEnvio === 2;
      const estaAgendado  = !todosEnviados && item.statusEnvio === 4;
      const temErro       = !todosEnviados && item.statusEnvio === 3;

      // Normalizar diabetes (string 'Sim'/'Não' ou boolean)
      let diabetes = false;
      if (typeof item.diabetes === 'boolean') {
        diabetes = item.diabetes;
      } else if (typeof item.diabetes === 'string') {
        diabetes = ['sim', 's', 'true', '1'].includes(item.diabetes.toLowerCase().trim());
      } else if (typeof item.diabetes === 'number') {
        diabetes = item.diabetes === 1;
      }

      const exames: ExameDetalhado[] = (item.exames || []).map((e: any) => ({
        iD_EXAME:       e.idExame,
        desC_EXAME:     e.nome    || '',
        cD_EXAME:       e.codigo  || '',
        cD_EXAME_DB:    e.codigo  || '',
        iD_GRUPO_EXAME: null,
        sigla:          '',
        material:       e.material || 'Soro',
      }));

      const nomePerfil = item.exames?.[0]?.grupo || '';

      return {
        id:            index + 1,
        idAgendamento: item.idAgendamento,
        data:          item.dataCadastro     ? item.dataCadastro.split('T')[0]     : '',
        nome:          item.nomePaciente     || '',
        cpf:           item.cpf             || '',
        diabetes,
        tratamento:    item.tipoTratamento   || '',
        horarioColeta: item.dataAgendamento  ? item.dataAgendamento.split('T')[0] : '',
        status:        todosEnviados ? 'enviado' : (temErro ? 'erro' : (estaAgendado ? 'agendado' : 'correto')),
        expandido:     false,
        exames,
        podeEditar:    !todosEnviados,
    ...
    this.todosPacientes = pacientesUI;
    this.pacientes = pacientesUI;
    this.pacientesSelecionados.clear();
  }

  /** @deprecated Mantido apenas para compatibilidade — usa carregarPaginado() */
  carregarPacientesLegado(): void {
    this.carregarPaginado();
  }
  ...
  /**
   * Retorna os pacientes da página atual (já filtrados/paginados pelo servidor).
   */
  ...
  /**
   * Aplica filtro de status e recarrega do servidor
   */
  ...
  /**
   * Aplica filtro de data e recarrega do servidor
   */
  ...
  /**
   * Limpa todos os filtros e recarrega do servidor
   */
  limparFiltros(): void {
  ...
  /**
   * Total de páginas calculado pelo total retornado pelo servidor.
   */
  ...
  /**
   * Retorna os pacientes da página atual (já paginados pelo servidor).
   */
  ...
  get estatisticas() {
    // Usa os totais globais vindos do servidor (resumo) para exibição no cabeçalho
    return {
      total:     this.totalRegistros,
      enviados:  this.resumoServidor.transferidos,
      pendentes: this.resumoServidor.pendente,
      erros:     this.resumoServidor.erros,
      agendados: this.resumoServidor.agendados,
    };
  }
   ...
   */
  onBuscaChange(): void {
    if (this.isSearchBlocked('paciente')) return;
    // Debounce de 400ms para não disparar uma requisição a cada tecla
